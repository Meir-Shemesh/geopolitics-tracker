...
- `src/reporting/synthesize.py` - שלב א' של Reporting: קריאת Sonnet יחידה ליום, מקבצת את מאמרי היום לפי נושא אמיתי (לא לפי `region_topic` הגולמי) לטבלאות `reports`/`report_sections`/`report_section_articles`, כולל רשת ביטחון כפולה (fallback section דטרמיניסטי + retry מבוסס-יחס) שמבטיחה כיסוי מלא של כל מאמר. ראו "החלטות ארכיטקטוניות קבועות". `reports.sources_included` (`get_sources_for_date()` ב-`db.py`) מחושב **פעם אחת כאן** מ-JOIN דרך `articles` בפועל (לא מ-`downloaded_files` בלבד - מקור שירד אך לא תרם אף מאמר, כמו PDF סרוק-בלבד, לא נכלל) ונשמר כעמודה - `render.py`/`publish.py` קוראים אותה כמו-שהיא, לא מחשבים מחדש.
