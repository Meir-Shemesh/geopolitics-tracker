...
- `src/reporting/render.py` - שלב ב' של Reporting: הופך שורות `report_sections` ל-4 קבצים (HTML+PDF × עברית+אנגלית) תחת `reports/{he,en}/`, כולל קידוד צבעוני לפי `category` (`category_css()` - פומבי, משוכפל לשימוש גם בדף הבית ב-`publish.py`), גופן Heebo מוטמע מקומית (`src/reporting/assets/fonts/`, מועתק אוטומטית גם ל-`reports/assets/fonts/`), ועוגן `id="section-{id}"` על כל section (כולל fallback) - נדרש כדי שקישורי-עומק מהמניפסט יעבדו. `build_nav_html(back_href, other_lang_href, lang)` מקבל hrefs מפורשים (לא גוזר מ-`report_date`) - כדי שגם `about.html` (ב-`publish.py`) יוכל לעשות בו שימוש חוזר.
...
- `src/publishing/publish.py` - שלב Publishing: בונה `docs/` (מקור ה-GitHub Pages) מתוך `reports/` - מעתיק HTML/PDF וגופן Heebo, בונה `archive.html` (ארכיון כרונולוגי, לכל שפה + עותק-שורש) ו-`about.html` (עמוד אתר רגיל, דו-לשוני) ו-`manifest.json` סטטי (`assets/data/manifest.json`, גם ב-`docs/` וגם ב-`reports/`), ובונה **דף בית אינטראקטיבי** (`index.html`, לכל שפה + עותק-שורש) - מפת עולם SVG מוטמעת inline (`src/publishing/assets/map/world.svg`, Public Domain, מועתקת גם כנכס עצמאי ל-`assets/map/`), ציר זמן, וכרטיס-דוח-אחרון, כולם JS+`fetch()` שקוראים מ-`manifest.json` בזמן ריצה. תמיד דורס, בלי `--force` (כמו `render.py`).
